...
class MyChessAI():
    ...
    def getScore(self, mychess, yourchess):
        ...
        if mychess[FIVE] > 0:        #玩家五子连珠（FIVE）
            return (10000, 0)
        if yourchess[FIVE] > 0:        #电脑五子连珠（FIVE）
            return (0, 10000)

        if mychess[SFOUR] >= 2:         #双四情况（SFOUR）    表示我方至少有两个双四
            mychess[FOUR] += 1          #将棋局中的四子棋子数量mychess[FOUR]加1

        if yourchess[FOUR] > 0:         #检查电脑是否有四子（FOUR）
            return (0, 9050)            #电脑有优势
        if yourchess[SFOUR] > 0:        #检查对方是否有的双四（SFOUR）
            return (0, 9040)
        if mychess[FOUR] > 0:           #检查自己的四子（FOUR）
            return (9030, 0)
        if mychess[SFOUR] > 0 and mychess[THREE] > 0:   #检查是否同时存在自己的双三（SFOUR）和三子（THREE）
            return (9020, 0)

        # 检查对方是否存在至少两个三子（THREE），而我方没有三子（THREE）和活三（STHREE）的情况
        if yourchess[THREE] > 0 and mychess[SFOUR] == 0:
            return (0, 9010)

        # 检查自己是否存在至少两个三子（THREE），而对方没有三子（THREE）和活三（STHREE）的情况
    ...
    #遍历四个方向，检查每个方向上的棋型，并更新计数器和记录。如果某个方向的记录已经存在，说明已经检查过了，此时只需对计数器进行相应的处理
    ...
    def getLine(self, board, x, y, direction, me, you):
        ...
        for i in range(9):
            tmp_x += direction[0]
            tmp_y += direction[1]
            if (tmp_x < 0 or tmp_x >= self.len or tmp_y < 0 or tmp_y >= self.len):          #检查当前位置是否超出了棋盘的范围。
                line[i] = you  # 出界
            else:
